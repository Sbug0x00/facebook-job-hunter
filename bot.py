...
def ket_noi_sheet():
    ...
    # Chỉ đọc trực tiếp từ 2 Tab bạn đã tạo sẵn, tuyệt đối không tự tạo để tránh lỗi cú pháp gspread
...
def viet_truyen(ten, tap, cot):
    ...
    return lam_sach_van_ban(truyen_hay)
...
def main():
    try:
        ...
        if tap_hien_tai > tong_so_tap:
            print("Bộ truyện đã hoàn thành tất cả các tập!")
            return

        tieu_de = f"[Tập {tap_hien_tai}/{tong_so_tap}] - {ten_truyen.upper()}"
        print(f"Đang chuẩn bị viết bài: {tieu_de}")
        ...
        if cookie_moi:
            ...
            print("Đăng bài thành công và đã cập nhật số tập tiếp theo.")
        else:
            print("Đăng bài thất bại, vui lòng kiểm tra lại Cookie Facebook.")
            
        if os.path.exists(file_anh):
            os.remove(file_anh)
    except Exception as e:
        print(f"Lỗi vận hành hệ thống: {e}")
...
def viet_truyen(ten, tap, cot):
    ...
    van_ban_sach = lam_sach_van_ban(truyen_hay)
    return van_ban_sach
...
def viet_truyen(ten, tap, cot):
    ...
    for char in ["**", "*", "__", "###", "##"]:
        truyen_hay = truyen_hay.replace(char, "")
